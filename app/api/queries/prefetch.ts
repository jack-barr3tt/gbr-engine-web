...
export const prefetchUseGetService = (queryClient: QueryClient, clientOptions: Options<GetServiceData, true> = {}) => queryClient.prefetchQuery({ queryKey: Common.UseGetServiceKeyFn(clientOptions), queryFn: () => getService({ ...clientOptions }).then(response => response.data) });
